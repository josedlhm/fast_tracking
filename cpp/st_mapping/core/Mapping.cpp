...
SemanticPointCloud ExtractPointCloud(const cv::Mat &rgb_img,
    ...
    points_labels.resize(num_valid_pixels);
    ...
    parallel_for(tbb::blocked_range<int>(0, num_valid_pixels),
                 [&](const tbb::blocked_range<int> &block) {
                     for (int i = block.begin(); i < block.end(); i++) {  // Access row-wise
                         ...
                         // Compute the point semantic
                         points_labels[i] = semantic_mask.at<uint8_t>(v, u);
                     }
                 });

    return std::make_pair(pcd, points_labels);
}

SemanticPointCloud VoxelDownSample(const PointCloud &pcd,
                                   const std::vector<int> &points_labels,
                                   const double voxel_size) {
    // Initialization
    tsl::robin_set<Voxel, VoxelHash> voxels;
    PointCloud pcd_dowsampled;
    std::vector<int> points_labels_downsampled;
    pcd_dowsampled.reserve(pcd.size());
    points_labels_downsampled.reserve(pcd.size());
    ...
    std::for_each(indices.cbegin(), indices.cend(), [&](const int idx) {
        ...
        const int label = points_labels[idx];
        const auto voxel = PointToVoxel(point, voxel_size);
        if (!voxels.contains(voxel)) {
            voxels.insert(voxel);
            pcd_dowsampled.emplace_back(pcd[idx]);
            points_labels_downsampled.emplace_back(label);
        }
    });
    pcd_dowsampled.shrink_to_fit();
    points_labels_downsampled.shrink_to_fit();

    return std::make_pair(pcd_dowsampled, points_labels_downsampled);
}
