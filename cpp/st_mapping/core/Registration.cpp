...
using Voxel = st_mapping::LocalMap::Voxel;
...
std::tuple<Eigen::Vector3d, double> GetClosestNeighbor(const Eigen::Vector3d &point,
                                                       const st_mapping::LocalMap &voxel_map) {
    ...
    std::for_each(neighbors.cbegin(), neighbors.cend(), [&](const auto &neighbor) mutable {
...
Sophus::SE3d CorrectPoseWithICP(const PointCloud &frame,
                                const LocalMap &map_pcd,
                                ...
                                int max_num_threads) {
    // Setup TBB parallelism
    max_num_threads = max_num_threads > 0 ? max_num_threads : tbb::info::default_concurrency();
    static const auto tbb_control_settings = tbb::global_control(
        tbb::global_control::max_allowed_parallelism, static_cast<size_t>(max_num_threads));

    if (map_pcd.Empty()) return initial_guess;

    // Equation (9)
    std::vector<Eigen::Vector3d> source;
    source.resize(frame.size());
    std::transform(frame.cbegin(), frame.cend(), source.begin(), [](const auto &colored_point) {
    ...
    for (int j = 0; j < max_num_iterations; ++j) {
        // Equation (10)
        const auto correspondences = DataAssociation(source, map_pcd, max_correspondence_distance);
        // Equation (11)
